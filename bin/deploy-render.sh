...
echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  🚀 部署 Kiro Travel 到 Render"
echo -e "${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ -z "$RENDER_DEPLOY_HOOK" ]; then
    echo -e "${RED}✗${NC} RENDER_DEPLOY_HOOK 未配置"
    echo ""
    echo "请在 $ENV_FILE 中配置 Deploy Hook URL"
    echo "获取方式: Render Dashboard > kiro-travel > Settings > Deploy Hook"
    exit 1
fi

echo -e "${BLUE}ℹ${NC} Deploy Hook URL: ${RENDER_DEPLOY_HOOK:0:50}..."
echo ""
